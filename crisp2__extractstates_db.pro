...
;
;-
pro crisp2::extractstates_db, strings, states, datasets = datasets, cam = cam
  ...
  for iset = 0, Nsets-1 do begin
    ...
    ;;   'science': datatype='data'
    ;; endcase
    if datatype eq 'polcal' then begin
      state = {CRISP2_POLCAL_STATE}
    endif else begin
      state = {CRISP2_STATE}
    endelse
    ...
    for iconf=1,Nconfs do begin
      ...
      for ifile = 0, Nbursts-1 do begin
        ...
        mosaic_pos = burst[10]    ; required for filename generation
        burst_id = burst[0]        
        ...
        if state.is_wb then begin
          state.fpi_state =  burst[7] + '_' + strtrim(string(tuning, format='(I+0)'),2)
          state.tuning = burst[7] + '_+0'
          state.tun_wavelength = state.pf_wavelength
        endif else begin
          state.tuning = burst[7] + '_' + strtrim(string(tuning, format='(I+0)'),2)
        ...
        if datatype eq 'polcal' then begin ;keyword_set(polcal)
          st = replicate( {CRISP2_POLCAL_STATE}, Nframes )
        endif else begin
          st = replicate( {CRISP2_STATE}, Nframes )
        endelse
        ...
        frame = strsplit(frames_ans[1],tab,/extract,/preserve_null)
        framenum = long(frame[3])
        lc_state = fix(frame[8])
        state.framenumber = long(framenum)
        if state.is_wb then begin
          state.fullstate = state.cam_settings + '_' + burst[9] + '_' + burst[7] + '_+0'
        endif else begin
          state.fullstate = state.cam_settings + '_' + burst[9] + '_' + burst[7] + '_' + strtrim(string(tuning, format='(I+)'),2)
        endelse
  ...
  if use_strings then begin
    if strmatch(strings[0],'*CRISP/data*') then begin
      ...
      for icam=0,Ncams-1 do begin
        ...
        match2, fs, files, suba
        if min(suba) eq -1 then begin
          print, 'There is no information for such file in the database.'
          print, '   files[0]: ', strings[0]
      ...
      match2, strings, states.filename, suba 
      if min(suba) eq -1 then begin
        print
        print, 'There is no information in the database for such files.'
        print, 'It could be because of wrong root_dir parameter in "config.txt" file.'
        print,'Check it!'
        dr = file_dirname(strings[0])
        print
        print, 'Directory of input files: '
        print, '    ', dr
        drr = file_dirname(states[0].filename)
        print
        print,'Directory for the database: '
        print, '    ', drr
        stop
      endif
      states = states[suba]
