...
; 
;-
pro red::make_cmap_intdif2, scan = scan, cam = cam, pref = pref, debug = debug, lc = lc
;
...
;
  mpref = state.pref
  upref = reform([mpref[uniq(pref, sort(pref))]])
  np = n_elements(upref)
     ...
     for cc = 0, 1 do begin
        ...
        for ww = 0L, nw - 1 do begin
           ...
           if(ww eq 0) then begin
              tmp = ((red_sumfiles(mfiles[idx]) - dd) * g)
              dim = size(tmp, /dim)
              cub = fltarr(nw,dim[0], dim[1])
              cub[ww,*,*] = temporary(tmp)
           endif else cub[ww,*,*] = ((red_sumfiles(mfiles[idx]) - dd)*g)
        ...
        for jj = 0, dim[1] - 1 do begin
           for ii = 0, dim[0] - 1 do begin
              f77_hintep, nw, udwav, cub[*,ii,jj], nw, udwav + cmap[ii,jj], tmp
              cub2[*,ii,jj] = tmp
           endfor
