...
pro red::clean_workdir, timestamps = timestamps, prefilters = prefilters
  ...
  case 1 of
    ...
    n_elements(prefilters) gt 0 : begin
      searchstring = [ 'flats/cam*.'+prefilters+'.????_[+-]*' $
                       , 'pinh_align/cam*.'+prefilters+'.????_[+-]*.pinh' $
                       , 'pinh_align/cam*.'+prefilters+'.????_[+-]*.fpinh' $
                       , 'polcal_cubes/cam*.'+prefilters+'.??.fits' $
                       , 'polcal_sums/Crisp-?/cam*.LP*.qw*.'+prefilters+'.????_[+-]*' $
                       , 'gaintables/cam*.'+prefilters+'.????_[+-]*' $
                       , 'gaintables/??:??:??/cam*.?????.'+prefilters+'.????_[+-]*.gain' $
...
a -> clean_workdir, prefilters = '6563'
