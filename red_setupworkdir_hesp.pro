...
; 
;    2023-11-21 : MGL. New version for HeSP based on the crisp2
;                 version.
; 
;-
pro red_setupworkdir_hesp, work_dir, root_dir, cfgfile, scriptfile, isodate $
...
;   if Ndirs gt 0 then instrument = 'CRISP2' else instrument = 'CRISP'
  ...
  ;; calib/ dd??=darks, ff??=flats, foc??=pinhole focus scans,
  ;; rmap=maps, pc??=polcal
  ...
  darksubdirs = red_find_instrumentdirs(root_dir, instrument, 'dd*', count = Ndarkdirs)
  flatsubdirs = red_find_instrumentdirs(root_dir, instrument, 'ff*', count = Nflatdirs)
...
;                                        , count = Nflatdirs)
  ...
  printf, Clun,'image_scale = 0.1105 ; Context image , sp is ~0.1306'
  
  printf, Clun,'diversity = 6.6e-3 ; [m]' ; Nominal value 

  if keyword_set(calibrations_only) then begin
    printf, Slun, 'a = hespred("'+cfgfile+'",/dev)' 
  endif else begin
    printf, Slun, 'a = hespred("'+cfgfile+'")' 
  endelse
  ...
  cams = file_basename(file_dirname(darksubdirs))
  cams = cams[uniq(cams,sort(cams))]
  ;; We want the context camera first!
  cams = reverse(cams)
...
;
  ...
  for i = 0, Ndirs-1 do begin
    pinhsubdirs = file_search(pinhdirs[i]+'/'+instrument+'*', count = Nsubdirs, /fold)
    if Nsubdirs gt 0 then begin
      ...
      for j = 0, Nsubdirs-1 do begin
        pinhsubsubdirs = file_search(pinhsubdirs[j]+'/'+instrument+'*' $
...
;                                          , count = Npolcalsubdirs)
  polcalsubdirs = red_find_instrumentdirs(root_dir, instrument, 'pc*', count = Npolcalsubdirs)
  
  if Npolcalsubdirs gt 0 then begin

    polcaldirs = file_dirname(file_dirname(polcalsubdirs))
...
;                                           , count = Nsubdirs)
  ...
  datadirs = file_search(root_dir + '/obs*', count = Ndatadirs)
  if Ndatadirs gt 0 then begin
    dirarr = red_strreplace(datadirs[uniq(datadirs, sort(datadirs))], root_dir, '')
...
;  if n_elements(dirarr) gt 0 then printf, Clun, "data_dir = ['"+strjoin(dirarr, "','")+"']"
  ...
  printf, Slun, ''
  printf, Slun, ';; -----------------------------------------------------'
  printf, Slun, ";; Now run Jaime's python scripts"
  printf, Slun, 'stop'          
  printf, Slun, ''
