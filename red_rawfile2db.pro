      ...
      for istate=0, Nstates-1 do begin           
        ...
        if state ne '' then begin         
          ...
          if iscan eq 0 then begin
            fquery = 'SELECT id FROM filters WHERE prefilter = ' + pref + ';'
            red_mysql_cmd, handle, query, ans, nl, debug=debug
            if nl eq 1 then begin
              fquery = 'INSERT INTO filters (prefilter, wavemax, wavemin, wavelnth, waveunit) VALUES(' + pref + ', ' + $
              ...
              red_mysql_cmd, handle, fquery, ans, nl, debug=debug
            endif
          endif                
        endif
        ...
        ;;query += 'SELECT LAST_INSERT_ID() INTO @burst_id;'
        query += 'SELECT id FROM bursts WHERE first_frame = "' + strtrim(string(dbinfo[ifile].FIRST_FRAME),2) + $
         '" AND config_id = "' + config_id + '" INTO @burst_id;'
