...
;
;   2026-07-01 : MGL. Ordinary subroutine, based on class method.
;                Remove the files keyword, new keyword developer_mode.
; 
;-
pro red_headerinfo_addstep, header $
                            , addlib = addlib $
                            , anchor = anchor $
...
;  print, 'Keyword: ', 'PRSTEP'+stp
  ...
  ;; Get version info explicitly as we don't have the class object.
  red_version_info , branch_pipeline = branch_pipeline $
  ...
  red_headerinfo_addlib, header, 'SSTRED', version_pipeline, prbranch = branch_pipeline
  red_headerinfo_addlib, header, 'IDLAstro', version_idlastro
  red_headerinfo_addlib, header, 'Coyote', version_coyote
  red_headerinfo_addlib, header, 'mpfit', version_mpfit
  red_headerinfo_addlib, header, 'reduxdlm', version_reduxdlm
  
  for ilib = 0, n_elements(addlib)-1 do red_headerinfo_addlib, header, addlib[ilib]

  ;; Procedure mode
  if keyword_set(developer_mode) then red_append, prmode, 'Developer mode' 
