...
function red_readhead_db, files, date_beg=date_beg, framenumbers=framenumbers, status=status
  ...
  for ins=0,1 do begin
    ...
    Nfls = n_elements(instr_indx)
    if Nfls eq 1 then if instr_indx eq -1 then continue ; no files with the instrument, switch to other one
    
    red_mysql_check, handle
    file_names = strarr(Nfls)
    dir_names = strarr(Nfls)
    sets = strarr(Nfls)
    
    for ifile=0, Nfls-1 do begin
      ii = instr_indx[ifile]
      file_names[ifile] = file_basename(files[ii])
      dir = file_dirname(files[ii])
      dir_names[ifile] = dir
    ...
    red_mkhdr,hdr, 2, [200,200] ;, /extend
...
;      red_fitsaddkeyword, hdr, 'DESCRIPTION', reply[3]
      ...
      if nl eq 1 then begin
        st.msg += 'There is no entry in configs table for ' + instrument + ' ' + date_time[iset] + ' ' + cameras[icam] + ' dataset.\r'
        st.code = -1
        status.add, st
        framenumbers.add, [0]
        date_beg.add, ['0']
      ...
      conf = strsplit(conf_ans[1],tab,/extract,/preserve_null)
      config_id = conf[0]
      ...
      if nl eq 1 then begin
        st.msg += 'There is no entry in detectors table for detector_id = ' + det_id + '.\r'
        st.code = -1
        status.add, st
        framenumbers.add, [0]
        date_beg.add, ['0']
      ...
      red_mysql_cmd,handle,query,det_ans,nl,debug=debug
      if nl eq 1 then begin
         st.msg += 'There is no entry in filename_templates table for fnm_templt_id = ' + tmplt_id + '.\r'
        st.code = -1
        status.add, st
        framenumbers.add, [0]
        date_beg.add, ['0']
      ...
      if nl eq 1 then begin
         st.msg += 'There is no entry in bursts table for the configuration ' + date_time[iset] + ' ' + conf[2] + '\r'
        st.code = -1
        status.add, st
        framenumbers.add, [0]
        date_beg.add, ['0']
      ...
      for ifile = 0, Nbursts-1 do begin
        ...
        if ins eq 0 then begin ; CHROMIS
          ...
          if cal_id ne 0 then begin ;  otherwise it can be darks ...               
            ...
            if nl eq 1 then begin
              st.msg += 'There is no entry in calibrations table for id ' + cal_id + '\r'
              st.code = -1
              status.add, st
              framenumbers.add, [0]
              date_beg.add, ['0']
            ...
            if nl eq 1 then begin
              st.msg += 'There is no entry in filters table for prefilter = ' + nbpref + '.\r'
              st.code = -1
              status.add, st
              framenumbers.add, [0]
              date_beg.add, ['0']
          ...
          v=execute(fnm_gen)
          if ~v then begin
            st.msg += 'Failed to generate filename \r'
            st.code = -1
            status.add, st
            framenumbers.add, [0]
            date_beg.add, ['0']
          ...
          if where(file_names eq fnm) eq -1 then begin
            print, ifile, '   ', fnm
          ...
          if nl eq 1 then begin
            st.msg += 'There is no entry in chromis_frames table for the burst_id ' + burst_id + '\r'
            st.code = -1
            status.add, st
            framenumbers.add, [0]
            date_beg.add, ['0']
          ...
          red_mysql_cmd,handle,query,frames_ans,nl,debug=debug
          if nl eq 1 then begin
            st.msg += 'There is no entry in ' + table + ' table for the burst_id ' + burst_id + '\r'
            st.code = -1
            status.add, st
            framenumbers.add, [0]
            date_beg.add, ['0']
