...
;+
; Sets up a work directory for the SLITJAW instrument.
;
; :Categories:
;
;   SST pipeline
...
;
;-
pro red_setupworkdir_slitjaw, work_dir, root_dir, cfgfile, scriptfile, isodate $
  ...
  red_metadata_store, fname = work_dir + '/info/metadata.fits' $
                      , [{keyword:'INSTRUME', value:'SLITJAW' $
  ...
  printf, Slun, 'a = slitjawred("'+cfgfile+'")'
  ...
  darksubdirs = red_find_instrumentdirs(root_dir, 'slit', '*dark*' $
                                        , count = Nsubdirs)
  ...
  flatsubdirs = red_find_instrumentdirs(root_dir, 'slit', '*flat*' $
                                        , count = Nsubdirs)
  if Nsubdirs gt 0 then begin
    ;; There are slitjaw flats!
  ...
  for i = 0, Ndirs-1 do begin
    pinhsubdirs = file_search(pinhdirs[i]+'/slit*', count = Nsubdirs, /fold)
    if Nsubdirs gt 0 then begin
      printf, Slun
...
;        printf, Slun, 'a -> sumpinh_new'
      ...
      for j = 0, Nsubdirs-1 do begin
        pinhsubsubdirs = file_search(pinhsubdirs[j]+'/slit*' $
...
;              printf, Slun, 'a -> sumpinh_new'
  ...
  for i = 0, Ndirs-1 do begin
    ...
    if total(sciencedirs[i] eq nonsciencedirs) eq 0 then begin
      sciencesubdirs = file_search(sciencedirs[i]+'/slit*' $
                                   , count = Nsubdirs, /fold)
      if Nsubdirs gt 0 then begin
        ...
        for j = 0, Nsubdirs-1 do begin
          sciencesubsubdirs = file_search(sciencesubdirs[j]+'/slit*' $
                                          , count = Nsubsubdirs, /fold)
          if Nsubsubdirs gt 0 then begin
