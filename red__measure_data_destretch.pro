pro red::measure_data_destretch, pref = pref, scan = scan, min = min, max = max, smooth = smooth, bad = bad, $
                                 ...
                                 t0 = t0, t1 = t1, scale = scale, tstep = tstep
 ;; Get procedure name
  ...
  if(file_test(ofile)) then restore, ofile
  ...
  for ss=0, nscan-1 do for ll = 0, n_elements(ulc)-1 do for ww = 0,n_elements(uwav)-1 do begin
...
;; Loop scans
  for ss = t0,t1 do begin
     ...
     if(ss eq 0) then begin
        ...
        ff = red_flat2gain(red_clipim(f0(ffile), cl[*,0]), mi = min, ma = max, smooth = smooth, bad = bad, /preserve)     
     endif ;; (cc eq 0)
     ...
     for ll = 0, n_elements(ulc)-1 do begin
        for ww = 0,n_elements(uwav)-1 do for nn=0, numadd[ww,ll,ss]-1 do begin
           ...
           shifts[*,nn,ww,ll] = round(red_shc(refs[*,*,ss], d[*,*,nn,ww,ll], /filt))
     ...
     save, file=ofile, corrs, shifts, ifiles, files, st, rms, $
           ...
           nscan, uscan, pref, upref, done, tile, clip
     done[ss] = 1
  endfor   ;; cc (Scans)
