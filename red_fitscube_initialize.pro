...
;
;    2019-09-12 : MGL. A version that is not a class method.
; 
;-
pro red_fitscube_initialize, filename, hdr, lun, fileassoc, dimensions, isodate = isodate, wcs = wcs
  ...
  if n_elements(isodate) eq 0 then begin
    ;; Get it from the header.
    isodate = fxpar(hdr, 'DATEREF', count = cnt)
    if cnt eq 0 then isodate = fxpar(hdr, 'DATE-BEG', count = cnt)
    if cnt eq 0 then stop
    isodate = (strsplit(isodate, 'T', /extract))[0]
  endif
  ...
  dateref = isodate+'T00:00:00.000000' ; Midnight
