...
;       MGL, 2013-06-03 : Changed so it uses coyote graphics. Renamed
;                         to mgl_timeaxis.pro 
;       MGL, 2014-10-16 : Renamed again, now to red_timeaxis.
; 
; 
; 
;
...
;-
;-------------------------------------------------------------
pro red_timeaxis, t, jd=jd, form=form, nticks=nticks, $
...
;	mxlines = 1
  ...
  cgplots, !x.crange, [yv, yv], color=color, thick=thick
  ...
  for i = 0, n_elements(v)-1 do begin       ; Major (Labeled) ticks.
     cgplots, [ix(i),ix(i)], [iy,iy+tickl], color=color, /dev, thick=thick
     if keyword_set(nolabels) then goto, skip
;	  mgl_xprint, /init, ix(i) ,iy-laboff*oneperc, /dev, $
     mgl_xprint, /init, ix(i) ,iyt-laboff*oneperc, /dev, $
...
;	    mxlines = mxlines > (j+1)
        mgl_xprint, txt, align=.5, color=color,y0=y0, /dev
...
skip:
  ...
  for i = 0, n_elements(v2)-1 do begin ; Minor ticks.
     cgplots, [ix(i),ix(i)], [iy,iy+tickl/2.], color=color,/dev,thick=thick
  endfor
  ...
  if n_elements(yvalue) eq 0 then begin
     ...
     cgplots, !x.crange, [0,0]+yv1, color=color,thick=thick
     tmp = convert_coord(v,v*0.+yv1,/data,/to_dev)
     ix = round(tmp(0,*))
     iy = round(tmp(1,*))
     for i = 0, n_elements(v)-1 do begin
        cgplots, [ix(i),ix(i)], [0,-tickl]+iy, color=color, /dev,thick=thick
     endfor
     tmp = convert_coord(v2,v2*0.+yv1,/data,/to_dev)
     ix = round(tmp(0,*))
     iy = round(tmp(1,*))
     for i = 0, n_elements(v2)-1 do begin ; Minor ticks.
        cgplots,[ix(i),ix(i)],[0,-tickl/2.]+iy,color=color,/dev,thick=thick
     endfor
  endif
  ...
  if n_elements(title) ne 0 then begin
     ...
     ix = tmp(0)
     iy = min(iymin)
     mgl_xprint,/init,ix,iy,/dev,size=size,dy=dy,charthick=charthick
     mgl_xprint,' ',/dev
     mgl_xprint,' ',/dev
;	  if laboff ge 0 then for i = 1, mxlines do mgl_xprint,' '
     mgl_xprint, title, align=.5, color=color, /dev
  endif
