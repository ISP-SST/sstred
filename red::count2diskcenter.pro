function red::count2diskcenter, pref, cam = cam
  ...
  if((pref EQ '8542') AND (self.descatter_dir NE '')) then begin
     ...
     ff = red_cdescatter(ff, bg, psf)
                                ...
                                ;
                                ; Now, we know the variation of the WB images on time
                                ; and the ratio between WB and NW at the time of the flats:
                                ; We can use ratios to obtain the correction factor for the NB.
                                ;
  var = aver / median(ff)
                                ;
  return, var
