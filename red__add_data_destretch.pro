pro red::add_data_destretch, scan = scan, min = min, max = max, smooth = smooth, $
  ...
  files = file_search(self.out_dir+'/calib_tseries/tseries.add.*idlsave', count = ct)
  ...
  for cc = 0, 2 do begin
     ...
     for ww = 0, n_elements(uwav)-1 do begin
        if(cc eq 0) then ffile = self.out_dir+'/flats/'+cams[cc]+'.'+pref+'.flat' $
  ...
  for tt = t0, t1 do begin
   ...
   ;  time = 0.0d0
     ...
     for jj = 0,3 do for ii = 0, 3 do begin
        ;; rotation
        if(rot) then begin
           itmat[ii,jj,*,*] = red_rotation(reform(itmat[ii,jj,*,*]), ang[tt])
           irmat[ii,jj,*,*] = red_rotation(reform(irmat[ii,jj,*,*]), ang[tt])
        endif
