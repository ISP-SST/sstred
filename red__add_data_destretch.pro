    ...
    for ww = 0, nwav-1 do begin
      for ll = 0, nlc-1 do begin
        for nn = 0, numadd[ww,ll,tt]-1 do begin
          ...
          inbt = red_fillpix(red_clipim((f0(tfiles[nn,ww,ll,tt]) $
                                         - dd[*,*,1]) * ff[*,*,ww,1], cl[*,1]), nt=nthreads)
          inbr = red_fillpix(red_clipim((f0(rfiles[nn,ww,ll,tt]) $
                                         - dd[*,*,2]) * ff[*,*,ww,2], cl[*,2]), nt=nthreads)
          nadd++
