...
;
;     2017-05-12 : MGL. Replace some more private library stuff.
;
; 
;-
function red_alignoffset, image, reference, cor, window = window, fitplane = fitplane
  ...
  if ~(si[1] eq sr[1] and si[2] eq sr[2]) then begin
  ...
  if keyword_set(fitplane) then begin
    ...
    xp = findgen(si[1]*2) # replicate(1.,si[1]*2) - si[1] ;    x_coord(si[1]/2, 1)
    yp = transpose(xp)
  ...
  if keyword_set(window) then begin
    if sr[1] ne sr[2] then stop ; Only square windows for now
    w = red_taper(sr[1]*[1, 0, 1/8.])
    image1 *= w
    reference1 *= w
  endif

  ;; Maximize cross-correlation over the integer pixels
  cor = float( fft( fft(image1, 1) * fft(reference1, -1), -1 ) )
  
  tmp = max(cor, s)
  x0 = s[0] mod si[1]
  x0 = x0 - si[1]*(x0 gt si[1]/2)
  
  y0 = s[0] / si[1]
  y0 = y0 - si[2]*(y0 gt si[2]/2)
  ...
  if n_params() ge 3 then begin
    ...
    w = i ge 0 && i le si[1]-1 && j ge 0 && j le si[2]-1.
