...
;+
; Modify or replace a FITS header in a file on disk.
; 
; Opens a FITS file, and adds or modifies a parameter in the FITS
; header, or replaces the header. Can be used for either the main
; header, or for an extension header. The modification is performed
; directly on the disk file.
;
; This is a modified version of fxhmodify.pro from the IDLAstro
; library. It lets you replace the entire header with a new header.
; 
; Restrictions: This routine can not be used to modify any of the
; keywords that control the structure of the FITS file, e.g. BITPIX,
; NAXIS, PCOUNT, etc. Doing so could corrupt the readability of the
; FITS file.
;
; Example: Modify the name 'OBJECT' keyword in the primary FITS header
; of a FITS file 'spec98.ccd' to contain the value 'test domeflat'
;
; IDL> fxhmodify, 'spec98.ccd', 'OBJECT', 'test domeflat'
;
; Side effects: If adding a record to the FITS header would increase
; the number of 2880 byte records stored on disk, then the file is
...
; :Author:
; 
;    William Thompson, GSFC, 3 March 1994 (as fxhmodify.pro)
; 
; 
; :Returns:
; 
...
;   name : in, type=string
;
;        Name of parameter. If NAME is already in the header the value
;        and possibly comment fields are modified. Otherwise a new
;        record is added to the header. If NAME is equal to either
;        "COMMENT" or "HISTORY" then the value will be added to the
;        record without replacement. In this case the comment
...
;
;      The '/' is added by this routine. Added starting in position
;      31. If not supplied, or set equal to '' (the null string), then
;      any previous comment field in the header for that keyword is
;      retained (when found).
; 
; 
; 
; :Keywords:
; 
;    extension : in, optional, type="integer or string"
;
;        Either the number of the FITS extension, starting with the
;        first extension after the primary data unit being one; or a
;        character string containing the value of EXTNAME to search
;        for. If not passed, then the primary FITS header is modified.
;
;    before : in, optional, type=string
;
;        Keyword string name.  The parameter will be placed before the
...
;
;    errmsg : out, optional, type=string
;
;         If defined and passed, then any error messages will be
;         returned to the user in this parameter rather than depending
;         on the MESSAGE routine in IDL. If no errors are encountered,
;         then a null string is returned. In order to use this
;         feature, ERRMSG must be defined first, e.g.
;
;               ERRMSG = ''
;               FXHMODIFY, ERRMSG=ERRMSG, ...
;               IF ERRMSG NE '' THEN ...
;
;    new_header : in, optional, type=strarr
;
;         If defined and passed, then ignore NAME, VALUE, and COMMENT.
;         Instead replace the old file header with the strarr given.
;
;  
; :History:
; 
...
;               Modified test for keyword EXTEND to only issue
;               warning.
;
;       Version 5, Mats Löfdahl, ISP, 11-Oct-2017  
; 
;-
PRO red_fitsmodheader, FILENAME, NAME, VALUE, COMMENT, BEFORE=BEFORE,   $
...
mkhdr, hdr, data
fxaddpar, hdr, 'TEST1', 'abc'
writefits, fname, data, hdr
...
;; Check data part
data2 = readfits(fname)
help, data, data2
print, stddev(data)
print, stddev(data-data2)
...
;; Check data part
data3 = readfits(fname)
help, data, data3
print, stddev(data)
print, stddev(data-data3)
