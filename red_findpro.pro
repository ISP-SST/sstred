pro red_FindPro, Proc_Name, NoPrint=NoPrint, DirList=DirList, ProList=ProList
;+
; NAME:
;     FINDPRO
; PURPOSE:
...
;
; PROCEDURES USED:
;       red_FDECOMP   -- Decompose file name
; REVISION HISTORY:
...
;
;       Renamed red_findpro for inclusion in the CRISPRED pipeline.
;       Use red_fdecomp rather than fdecomp. MGL 2013-07-11.
;
;-
;/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
...
; Use FILE_PATH() to search all directories for <name>.pro or <name>.sav files 
   ...
   ProList = file_search(path_dir + psep + name + '.{pro,sav}', COUNT=Nfile) 
      ...
      if (Nfile ge 1) then begin                     ;Found by FILE_SEARCH?
       red_fdecomp, ProList, ddisk,ddir,fname,ext
       dirlist = ddisk + ddir
       found = 1b
