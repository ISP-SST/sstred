...
pro red::rawdir2db, timestamp, cams = cams
  ...
  for icam = 0, Ncams-1 do begin
    ...
    dir = self.root_dir + '/' + 'CHROMIS-*/' + timestamp + '/' + cams[icam] + '/'
...
;              , SCANNUMBER     : 0L          $
    ...
    for ifile = 0, Nfiles-1 do begin
      ...
      if s.is_wb then begin
        ...
        ;; Speedup? We could get the state and tuning info for NB from
        ;; the STATE header keyword instead of getting it from the s
        ;; struct from extractstates.
...
;      if n_elements(s.GAIN          ) then dbinfo[ifile].GAIN           = s.GAIN            
      ...
      value = fxpar(h, 'CAMERA'  , count=cnt) & if cnt eq 1 then dbinfo[ifile].CAMERA   = value 
      ...
      value = fxpar(h, 'DETECTOR', count=cnt) & if cnt eq 1 then dbinfo[ifile].DETECTOR = value 
      ...
      value = fxpar(h, 'FILENAME', count=cnt) & if cnt eq 1 then dbinfo[ifile].FILENAME = value        
